...
class User(object):
    ...
    def repack73(self): #with 02 for zk8 (size73)
        ...
        return pack("<BHB8s24sIB7sx24s", 2, self.uid, self.privilege,self.password.encode('gbk'), self.name.encode('gbk'), self.card, 1, self.group_id.encode('gbk'), str(self.user_id).encode('gbk'))
    def __str__(self):
