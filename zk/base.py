    ...
    def set_user(self, uid, name, privilege=0, password='', group_id='', user_id='', card=0):
        ...
        if self.user_packet_size == 28:  # self.firmware == 6:
            if not group_id:
                group_id = 0
            try:
                command_string = pack('HB5s8sIxBHI', uid, privilege, password.encode(), name.encode(), card,
                                      int(group_id), 0, int(user_id))
            except Exception as e:
            ...
            command_string = pack('HB8s24s4sx7sx24s', uid, privilege, password.encode(), name_pad, card_str,
    ...
    def get_users(self):  # ALWAYS CALL TO GET correct user_packet_size
        ...
        if self.user_packet_size == 28:
            while len(userdata) >= 28:
                ...
                name = (name.split(b'\x00')[0]).decode(encoding='gbk',errors='ignore').strip()
